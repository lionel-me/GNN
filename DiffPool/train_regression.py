...
from dgl.data import tu,QM9Dataset

from model.cost_model import DiffPool
...
def graph_classify_task(prog_args):
    ...
    dataset = QM9Dataset(['mu'],cutoff = 2)
    ...
    if prog_args.load_epoch >= 0 and prog_args.save_dir is not None:
        model.load_state_dict(torch.load(prog_args.save_dir + "/" + prog_args.dataset
                                         + "/model.iter-" + str(prog_args.load_epoch)),strict = False)
...
def train(dataset, model, prog_args, same_feat=True, val_dataset=None):
    ...
    early_stopping_logger = {"best_epoch": -1, "val_acc": 10}
    ...
    for epoch in range(prog_args.epoch):
        ...
        for (batch_idx, (batch_graph, graph_labels)) in enumerate(dataloader):
            ...
            ape += torch.sum(torch.abs((graph_labels - ypred)))
...
def evaluate(dataloader, model, prog_args, logger=None):
    ...
    with torch.no_grad():
        for batch_idx, (batch_graph, graph_labels) in enumerate(dataloader):
            ...
            ape += torch.sum(torch.abs((graph_labels - ypred)))
    result = ape / (len(dataloader) * prog_args.batch_size)
    return result
